...
class mLSTM(Recurrent):
    ...
    def build(self, input_shape):
        ...
        self.kernel = self.add_weight(shape=(self.input_dim, self.units * 4),
                                      ...
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        self.m_kernel = self.add_weight(shape=(self.input_dim, self.units),
                                      name='m_kernel',
                                      initializer=self.kernel_initializer,
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
    ...
    def step(self, inputs, states):
        ...
        assert self.implementation == 2
        x = inputs * dp_mask[0]
        z = K.dot(x, self.kernel)
        h = h_tm1 * rec_dp_mask[0]
        xm = K.dot(x, self.m_kernel)
        hm = K.dot(h, self.m_recurrent_kernel)
        m = xm * hm
        z += K.dot(m, self.recurrent_kernel)
        if self.use_bias:
            z = K.bias_add(z, self.bias)
